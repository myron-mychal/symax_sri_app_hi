...
#define TIME_BETWEEN_STAGGERED_HI_TX_BLOCKS 50	// time between staggered blocks of UP messages being transmitted for harmonic injection
...
unsigned char readMotorDataCommands[] = {BUS_VOLTS_CMD, MOTOR_STATUS_CMD, MOTOR_DIR_CMD, ELECTRICAL_INFO_CMD, MEASURED_SPEED_CMD, THERMO_MECHANICAL_INFO_CMD};
uint16_t readMotorDataTimeInterval_u16[] = {500, 500, 500, 500, 100, 500};
...
{ 
  ...
  case SpdUpdate:
    {
      ...
      speedTx[7] = (unsigned char) (motor_Com_Control.motor_Metering_Data.motor_Actual_Direction_u8);
  ...
  case HarmonicUpdate:
	{
	  ...
	  if(blocks_to_transmit_u8 == 0)
	  {
		...
		tt_HarmonicUpdateTime = getSysCount() + TIME_BETWEEN_STAGGERED_HI_TX_BLOCKS;
      	return_state_u8 = SpdUpdate;  // revert back to AppStart state
                harmonic_data_cnt++;
  ...
  case startMotor:
    { // To start motor send SetSpeed command
      ...
      if((*digitalInputControl_motorCom_ptr).digitalInputs_Data.is_invertDirection)
      {
        if(motor_Com_Control.motor_Setting.motor_Direction_u8 == CCW) 
        {
          motor_Com_Control.motor_Metering_Data.motor_Actual_Direction_u8= CW;
        } else
        {
          motor_Com_Control.motor_Metering_Data.motor_Actual_Direction_u8 = CCW;
        }
      } else
      {
        motor_Com_Control.motor_Metering_Data.motor_Actual_Direction_u8 = motor_Com_Control.motor_Setting.motor_Direction_u8;
      }
    ...
    {   
      ...
      { // Ramp down speed to stop the motor
        ...
        { //issue final stop command and set time delay
          ...
          speedLen = sizeof(speedTx1);
          speedTx1[7] = (unsigned char) (motor_Com_Control.motor_Metering_Data.motor_Actual_Direction_u8);
...
void init_Motor_Setting(){
  motor_Com_Control.motor_Setting.control_Mode_u8 = 0;      //00 = Speed 01 = torque;
  ...
  motor_Com_Control.motor_Setting.min_Speed_u16 = 600;       //minumum speed in RPM 0dp
  motor_Com_Control.motor_Setting.hysteresis_Speed_u16 = 0; //hysteresis speed in RPM 0dp  
  motor_Com_Control.motor_Setting.max_Torque_u16 = 10;        //Maximum allowed torque
  motor_Com_Control.motor_Setting.min_Torque_u16 = 0;       //Minmium torque Nm
  motor_Com_Control.motor_Setting.motor_Direction_u8 = CCW;  // Default motor direction
  motor_Com_Control.motor_Metering_Data.motor_Actual_Direction_u8 = motor_Com_Control.motor_Setting.motor_Direction_u8;
...
void send_Periodic_Data_Request(uint8_t cmd_index_u8)
{
  ...
  readMotorDataCmdTx[5] = (unsigned char) ((uint16_t)readMotorDataTimeInterval_u16[cmd_index_u8]);
...
void MotorCom_UpdateMotorDirection(uint16_t direction) {
  motor_Com_Control.motor_Setting.motor_Direction_u8 = direction ? CW: CCW;
}
