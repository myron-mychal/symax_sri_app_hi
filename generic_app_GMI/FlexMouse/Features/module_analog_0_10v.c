...
uint8_t analog_increasing_debounce_count_u8 = 0; // Count to check if analog volts is above debounce value
uint8_t analog_decreasing_debounce_count_u8 = 0; // Count to check if analog volts is below debounce value
uint16_t analog_volts_counts_u16 = 0;            // Temporary storage for ADC value
uint8_t analog_low_alarm_enable_count_u8 = 0;    // Time when analog module init is complete
                           ...
                           uint8_t irq_identifier_u8)
{
  ...
  case INIT_MODULE:
    {     
      ...
      analog_low_alarm_enable_count_u8 = analogVolts_Control.analogVolts_Setting.analogLowAlarmEnableCount_u8; // delay before analog low voltage alarm can be triggered
  ...
  case RUN_MODULE:                                                             
    {      
      ...
      if (getSysCount() >= module_analog_0_10v_poll_time_u64) 
      {
        ...
        if(analog_low_alarm_enable_count_u8 > 0){
          analog_low_alarm_enable_count_u8--;
        } 
        if(analog_low_alarm_enable_count_u8 == 0) // On power up wait for few sec to detect correct analog input
          {
        ...
        analogVolts_Control.analogVolts_Data.analogErrorCode_u8 = (*adc1_LocalControl).adc1_ResultAvg.errorCode_u8; // Get any error codes
        DebounceAnalogVolts(); // Process the analog voltage
        
        // Check if analog is low. If enabled and analog is low, demand is "0"
        if ((analogVolts_Control.analogVolts_Data.is_analogLow == TRUE) && (analogVolts_Control.analogVolts_Setting.is_analogLowEnable == TRUE) && (analogVolts_Control.analogVolts_Setting.is_analogFailSafeEnable == FALSE))
        {
          analogVolts_Control.analogVolts_Data.analogDemand_u16 = 0;
          analogVolts_Control.analogVolts_Data.analogDemandPercent_u16 = 0;
          analogVolts_Control.analogVolts_Data.is_demandOn = FALSE;
          analogVolts_Control.analogVolts_Data.is_lowerHysteresisEnable = FALSE;
...
*/
void Init_Analog_Setting(void){  
  analogVolts_Control.analogVolts_Setting.onVolts_u16 = 205;               // Demand turn ON volts in counts of ADC
  analogVolts_Control.analogVolts_Setting.offVolts_u16 = 164;              // Demand turn OFF volts in counts of ADC. Acts as Hysteresis
  analogVolts_Control.analogVolts_Setting.minVolts_u16 = 300;              // Min volts correspodning to min demand
  analogVolts_Control.analogVolts_Setting.maxVolts_u16 = 1284;             // Max volts corresponding to max demand  
  analogVolts_Control.analogVolts_Setting.maxAdcCounts_u16 = 4095;         // Max output of ADC. 12 bit ADC left sifted by 4
  analogVolts_Control.analogVolts_Setting.debounceThresholdCounts_u16 = 21;// Change demand if the change in analog is outside this threshold. In ADC counts
  ...
  analogVolts_Control.analogVolts_Setting.voltsHysteresis_u16 = 21 ;       // Hys volts at the top end of demand. In ADC counts
  analogVolts_Control.analogVolts_Setting.minDemand_u16 = 13107;           // Min allowed demand. 20%
  ...
  analogVolts_Control.analogVolts_Setting.analogLowVolts_u16 = 250;        // Volts below which the bAnalog_Loss_Enable is set. In ADC counts
  ...
  analogVolts_Control.analogVolts_Setting.analogLowAlarmEnableCount_u8 = 100;   // analogPeriod*count delay before analog low volts alarm is detected in mSec. 
  analogVolts_Control.analogVolts_Setting.debounceLoopCount_u8 = 4;             // Debounce count
  analogVolts_Control.analogVolts_Setting.analogGain_f = 0.08056640625;         // Convert ADC coutns to volts 100*3.3/4096
  
  analogVolts_Control.analogVolts_Setting.is_enableAnalog = TRUE;          // If set to "0", 0V = Max demand 10V= 0 Demand
  analogVolts_Control.analogVolts_Setting.is_invertAnalog = FALSE;         // If set to "0", 0V = Max demand 10V= 0 Demand
  analogVolts_Control.analogVolts_Setting.is_analogLowEnable = FALSE;       // Enable analog low detect if set to "1"
  analogVolts_Control.analogVolts_Setting.is_analogFailSafeEnable = FALSE; // If set to "1", switch to fail safe demand when anlaog low is detected  
  analogVolts_Control.analogVolts_Setting.is_analogMinDemandEnable = FALSE;// If "True", min demand is maintaned when volts is below minVolts and above onVolts
  
  analogVolts_Control.analogVolts_Setting.inputMode_u8 = ANALOG_MODE;  // 0 = Analog input; 1 = digital input; 
...
void InitAnalogVoltsInputParameters(void)
{
  ...
  analogVolts_Control.analogVolts_Data.increasingIntercept_u16 = 0;    // Intercept used to calculate demand when analog is increasing
  analogVolts_Control.analogVolts_Data.decreasingIntercept_u16 = 0;    // Intercept used to calcualte demand when analog is decreasing
  
  analog_low_alarm_enable_count_u8 = analogVolts_Control.analogVolts_Setting.analogLowAlarmEnableCount_u8;
  ...
  analogVolts_Control.analogVolts_Data.increasingIntercept_u16 = (float)(analogVolts_Control.analogVolts_Setting.minDemand_u16 - (float)((analogVolts_Control.analogVolts_Data.increasingSlope_f) * (analogVolts_Control.analogVolts_Setting.minVolts_u16))); // Interecept used to calculate demand when analog is increasing
  
  // Assumes 0 V = 0 demand; Max volts = Max demand  
  analogVolts_Control.analogVolts_Data.is_analogLow = FALSE;             // Set to "TRUE", if analog low is detected
  analogVolts_Control.analogVolts_Data.is_decreasingAnalog = FALSE;       // Set to "TRUE", if analog voltage is decresing
  analogVolts_Control.analogVolts_Data.is_lowerHysteresisEnable = FALSE;  // Set to "TRUE", if lower end hys is enabled
  analogVolts_Control.analogVolts_Data.is_upperHysteresisEnable = FALSE;  // Set to "TRUE", if upper end hys is enabled 
  analogVolts_Control.analogVolts_Data.is_demandOn = FALSE;               // Set to"TRUE", if demand goes from 0 to above min_Demand_u16.  
}
...
void DebounceAnalogVolts(void){
  ...
  {  // Change is analog is above debounce
    ...
    { // Analog is decreasing
      analog_decreasing_debounce_count_u8++;
      analog_increasing_debounce_count_u8 = 0;
    } else if (analogVolts_Control.analogVolts_Data.analogVoltsCountsStable_u16  <  analog_volts_counts_u16)
    { // Analog is increasing
      analog_increasing_debounce_count_u8++;
      analog_decreasing_debounce_count_u8 = 0;
    } 
    
    // Check debounce loop count
    if ( (analog_increasing_debounce_count_u8 > analogVolts_Control.analogVolts_Setting.debounceLoopCount_u8) || (analog_decreasing_debounce_count_u8 > analogVolts_Control.analogVolts_Setting.debounceLoopCount_u8)  )
    {
      ...
      if(analog_decreasing_debounce_count_u8 > analogVolts_Control.analogVolts_Setting.debounceLoopCount_u8) 
      {
        analogVolts_Control.analogVolts_Data.is_decreasingAnalog = TRUE;
      } else
      {
        analogVolts_Control.analogVolts_Data.is_decreasingAnalog = FALSE;
      }
      analog_decreasing_debounce_count_u8 = 0;
      analog_increasing_debounce_count_u8 = 0;
    }
    ...
    {
      analogVolts_Control.analogVolts_Data.is_digitalInputON = FALSE;
    } else{
      analogVolts_Control.analogVolts_Data.is_digitalInputON = TRUE;
    }
    ...
    { // Check if analog volts is low
      ...
      {
        analogVolts_Control.analogVolts_Data.is_analogLow = TRUE;
      } else {
        analogVolts_Control.analogVolts_Data.is_analogLow = FALSE;
      }
    } else{
...
uint16_t GetAnalogDemand(void){
  ...
  if (analogVolts_Control.analogVolts_Setting.inputMode_u8 != ANALOG_MODE)
  {
    analog_demand_u32 = 0;
    analogVolts_Control.analogVolts_Data.is_demandOn = FALSE;
  } else if( (analogVolts_Control.analogVolts_Setting.is_analogLowEnable == TRUE) && (analogVolts_Control.analogVolts_Setting.is_analogFailSafeEnable == TRUE ) && (analogVolts_Control.analogVolts_Data.is_analogLow == TRUE)) { 
    // Check if Loss of analog
    if((analogVolts_Control.analogVolts_Data.is_demandOn == FALSE) && (is_volts_above_loss_analog == FALSE))//demand is never gone above min demand. 
    {  // Analog never crossed turned ON volts
      ...
      analogVolts_Control.analogVolts_Data.is_demandOn = TRUE;
      analogVolts_Control.analogVolts_Data.is_upperHysteresisEnable = FALSE;
    }
  } else {
    // If invert is enabled invert measured value
    if (analogVolts_Control.analogVolts_Setting.is_invertAnalog == TRUE) 
    {
    ...
    if (analogVolts_Control.analogVolts_Data.is_decreasingAnalog == FALSE)
    { // Slope used when analog value is increasing
      slope_f = analogVolts_Control.analogVolts_Data.increasingSlope_f;
      intercept_u16 = analogVolts_Control.analogVolts_Data.increasingIntercept_u16;
    } else {
      // Slope used when analog value is decreasing
      slope_f = analogVolts_Control.analogVolts_Data.decreasingSlope_f;
      intercept_u16 = analogVolts_Control.analogVolts_Data.decreasingIntercept_u16;
    }
    ...
    if ((analog_volts_u16 >= analogVolts_Control.analogVolts_Setting.offVolts_u16) && (analog_volts_u16 < (analogVolts_Control.analogVolts_Setting.maxVolts_u16 - analogVolts_Control.analogVolts_Setting.voltsHysteresis_u16)))
    {
      ...
      if (analog_volts_u16 >= analogVolts_Control.analogVolts_Setting.onVolts_u16) {
        ...
        {          
          ...
          analogVolts_Control.analogVolts_Data.is_lowerHysteresisEnable = TRUE;
          ...
          if((analogVolts_Control.analogVolts_Setting.is_analogMinDemandEnable) == TRUE && ( analogVolts_Control.analogVolts_Data.is_lowerHysteresisEnable == TRUE)){
          ...
          } else if( (analog_volts_u16 >= (analogVolts_Control.analogVolts_Setting.minVolts_u16 - analogVolts_Control.analogVolts_Setting.minDemandHysteresis_u16)) && ( analogVolts_Control.analogVolts_Data.is_lowerHysteresisEnable == TRUE)){
            // Apply lower end hysteresis
            analog_demand_u32 = (uint32_t)((uint64_t)(slope_f * analog_volts_u16) + intercept_u16);
          } else{
            if ((analogVolts_Control.analogVolts_Setting.is_invertAnalog == TRUE) && (analogVolts_Control.analogVolts_Setting.is_analogMinDemandEnable == TRUE))
            {
              analog_demand_u32 = analogVolts_Control.analogVolts_Setting.minDemand_u16;
            } else{
    ...
    } else if (analog_volts_u16 >= (analogVolts_Control.analogVolts_Setting.maxVolts_u16 - analogVolts_Control.analogVolts_Setting.voltsHysteresis_u16)) {
      // Part 3: 
      // upperHysteresis till maxVolts
      
      // Analog voltage above upper hysteresis voltage      
      if (analogVolts_Control.analogVolts_Data.is_upperHysteresisEnable  == TRUE)
      { // Analog already crossed hysteresis point
        if (analogVolts_Control.analogVolts_Data.is_decreasingAnalog == TRUE)
    ...
    if (analog_demand_u32 < analogVolts_Control.analogVolts_Setting.minDemand_u16)
    {
      ...
      analogVolts_Control.analogVolts_Data.is_demandOn = TRUE; // Demand greater than min demand.  
      analogVolts_Control.analogVolts_Data.is_lowerHysteresisEnable = TRUE; // Enable Hysteresis
    }
