...
{ 
  uint8_t  outputFunction[TOTAL_DIGITAL_OUTPUTS];     // Function of each digital output
  uint8_t  outputFunctionMode[TOTAL_DIGITAL_OUTPUTS]; // Function mode of each digital output. Fault output can be set in 3 different modes
  uint8_t  pwmFreqOutputScaleFactor_u8;      // Scale factor for tachOuputFrequency = speed * tachScaleFactor
...
{ 
  ...
  //uint16_t stateFlagRegister_u16;   // Register used to set LED based on state of motor
  uint8_t  is_relayOutputEnable;      // Enable Relay output set by user
};
