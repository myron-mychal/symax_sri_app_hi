...
/** Firmware update to Target data  **/
...
{ 
  ...
  case AppInit:                                                              //initial stage
    {     
      ...
      motor_FW_Update_Control.WrBufBlkNO = 0;
...
uint8_t FWWrStateMachine(void)
{
  ...
  case WrFw_WithOutErase:                   /*******************************  Write target whole firmware stage ****************************************/
    { 
      if(motor_FW_Update_Control.WrBufBlkNO != previousWrittenBlkNO)
      {
        if(motor_FW_Update_Control.WrBufBlkNO >= 0 )
        {//start a new block write
          motor_FW_Update_Control.Rd_Wr_busy = TRUE;
          previousWrittenBlkNO = motor_FW_Update_Control.WrBufBlkNO;
          _txFWdata= (uint8_t*) malloc(2);
        ...
        {//the whole write process have been done
          motor_FW_Update_Control.Rd_Wr_busy = FALSE;
  ...
  case WrFwAddr:
    { //ADDR_FLASH_PAGE_0
      _txFWdata= (uint8_t*) malloc(5);
      uint32_t blkStartAddr = ADDR_FLASH_PAGE_0 + (motor_FW_Update_Control.WrBufBlkNO * 256);
...
uint8_t FWBlkRead(uint8_t* _FW_RdDatBuf)
{
  ...
  case FwRdAddr:
    { //ADDR_FLASH_PAGE_0
      _txFWdata= (uint8_t*) malloc(5);
      uint32_t blkStartAddr = ADDR_FLASH_PAGE_0 + (motor_FW_Update_Control.RdBufBlkNO * 256);
      _txFWdata[0] = (uint8_t)(blkStartAddr >> 24);                 
      _txFWdata[1] = (uint8_t)(blkStartAddr >> 16); 
      _txFWdata[2] = (uint8_t)(blkStartAddr >> 8); 
      _txFWdata[3] = (uint8_t)blkStartAddr;
