...
void MotorDemandMux_InitSystem(void) {
  ...
  motorDemandMux_Control.motorDemandMux_Settings.control_Mode_u8 = DEMAND_MODE_SPEED; 
  motorDemandMux_Control.motorDemandMux_Settings.modbusPriority_u8 = 2;         // Demand priority. 1 being highest
  motorDemandMux_Control.motorDemandMux_Settings.analog0_10vPriority_u8 = 1;   // Demand priority. 1 being highest
  motorDemandMux_Control.motorDemandMux_Settings.digitalInputsPriority_u8 = 3; // Demand priority. 1 being highest
  motorDemandMux_Control.motorDemandMux_Settings.pwmInputPriority_u8 = 4;      // Demand priority. 1 being highest
  motorDemandMux_Control.motorDemandMux_Settings.analog4_20mAPriority_u8 = 5;  // Demand priority. 1 being highest  
  motorDemandMux_Control.motorDemandMux_Settings.maxPriority_u8 = 2;            // Demand priority. 1 being highest
...
void MotorDemandMux_Update(void) {
  ...
  if (demand_source_index_u16 != MOTOR_DEMAND_SOURCE_PRIORITY_SYSTEM) {
    ...
    motorDemandMux_Control.motorDemandMux_Data.currentDemandSource_u8 = active_demand_source_u16;
  }
...
#ifdef MODULE_DIGITAL_INPUTS_ID  
  if((*motorDemandMux_digitalInputsControl_ptr).digitalInputs_Data.is_motorEnabled == FALSE) 
  {
    ...
    case MOTOR_DEMAND_SOURCE_SERIAL_COMMAND:
      {
        ...
        } else if (motorDemandMux_ModbusSpeed && motorDemandMux_ModbusSpeed < 0xFFFF) // Ignore 0xFFFF speed from Modbus since its not valid
        {
...
#ifdef MODULE_DIGITAL_INPUTS_ID // Needed this for the else part of if((*motorDemandMux_digitalInputsControl_ptr).digitalInputs_Data.is_motorEnabled == FALSE). This makes sure digital inputs module is available 
  //if ( ((*motorDemandMux_digitalInputsControl_ptr).digitalInputs_Data.is_invertDirection == TRUE) && (motorDemandMux_Control.motorDemandMux_Settings.control_Mode_u8 == DEMAND_MODE_SPEED) )
    //{
...
uint16_t MotorDemandMux_GetSourceByPriority(void) {
  ...
  for(uint8_t priority_u8 = motorDemandMux_Control.motorDemandMux_Settings.maxPriority_u8; priority_u8 > 0; priority_u8--)
  { // Loop through lowest (max_Priority_u8) to highest priority (1)
      #ifdef MODULE_MODBUS_ID
      if(motorDemandMux_Control.motorDemandMux_Settings.modbusPriority_u8 == priority_u8)
      {
      ...
      #ifdef MODULE_ANALOG_0_10V_ID
      if( motorDemandMux_Control.motorDemandMux_Settings.analog0_10vPriority_u8 == priority_u8)
      {
      ...
      #ifdef MODULE_DIGITAL_INPUTS_ID
      if( motorDemandMux_Control.motorDemandMux_Settings.digitalInputsPriority_u8 == priority_u8)
      {
      ...
      #ifdef MODULE_PWM_INPUT_ID
      if( motorDemandMux_Control.motorDemandMux_Settings.pwmInputPriority_u8 == priority_u8)
      {
      ...
      #ifdef MODULE_ANALOG_4_20MA_ID
      if( motorDemandMux_Control.motorDemandMux_Settings.analog4_20mAPriority_u8 == priority_u8)
      {
...
void MotorDemandMux_ModbusUpdate(uint16_t commanded_speed, uint16_t commanded_demand, uint16_t commanded_start, uint16_t demand_source, uint16_t direction) {
  ...
  if (demand_source < MOTOR_DEMAND_SOURCES_TOTAL) {// todo: magic number - NO_DEMAND_SOURCE
    ...
    motorDemandMux_Control.motorDemandMux_Data.currentDemandSource_u8 = demand_source;
  }
