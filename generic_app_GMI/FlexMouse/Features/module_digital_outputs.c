...
void InitDigitalOutputsSetting(void){ 
  ...
  digital_Outputs_Control.digital_outputs_Setting.pwmOutFreq_u16 = 50; // Output frequency for duty cycle mode
  digital_Outputs_Control.digital_outputs_Setting.outputEnable_u16 = 0xffff;
  digital_Outputs_Control.digital_outputs_Setting.outputFunction[0] = PWM_DUTY_CYCLE_OUTPUT; //PWM_DUTY_CYCLE_OUTPUT or FAULT_OUTPUT;
  ...
  digital_Outputs_Control.digital_outputs_Setting.outputFunctionMode[0] = MODE_3; // 
  ...
  digital_Outputs_Control.digital_outputs_Setting.outputFunctionMode[3] = MODE_1; // NO when (low) during normal operation. NC (high) when faulted
  ...
  digital_Outputs_Control.digital_outputs_Setting.outputPolarity_u16 = 0x0004; // Active Low  
  digital_Outputs_Control.digital_outputs_Setting.pwmFreqOutputScaleFactor_u8 = 1; // value * speed = Output frequency
...
void ProcessRelayOut(uint8_t output_num_u8){
  ...
  case MODE_2: // User Enable Mode
    {
      if(digital_Outputs_Control.digital_outputs_Data.is_relayOutputEnable)
      {
        state_u8 = ENABLE;
      } else{
        state_u8 = DISABLE;
  ...
  case MODE_3: // Speed Threshold Mode
    {
      demand_percent_u16 = (*digitalOutputs_MotorCom_Control_ptr).motor_Metering_Data.demand_Output_Percent_u16;
      if( demand_percent_u16 > digital_Outputs_Control.digital_outputs_Setting.relayEnableDemandThreshold_u16)
      {
        state_u8 = ENABLE;
      } else{
        state_u8 = DISABLE;
...
void ProcessPwmFrequencyOutput(uint8_t output_num_u8){  
  ...
  case MODE_1: // Measured speed output
    {
      measuredSpeed_u16 = (*digitalOutputs_MotorCom_Control_ptr).motor_Metering_Data.demand_Output_Speed_u16;
      if(measuredSpeed_u16 == 0)
      {
        pwm_freq_u16 = 0;
      } else {
        pwm_freq_u16 = (uint16_t)(measuredSpeed_u16 * digital_Outputs_Control.digital_outputs_Setting.pwmFreqOutputScaleFactor_u8);
      }
      break;
    }
